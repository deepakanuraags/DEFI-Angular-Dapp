...
contract defitokentestMain is Ownable {
    using SafeMath for uint256;
    ...
    mapping(uint256 => mapping(int256 => DailyDataStore)) public dailyData;

    IERC20 public defitokentest;

    address public devaddr;

    uint256 public bonusEndBlock;

    uint256 public defitokentestPerBlock;

    uint256 public constant BONUS_MULTIPLIER = 1;

    int256 public currentday = -1;
    ...
    uint256 private rewardDuration = 60 seconds;
    ...
    function getNextGlobalEmissionDate() public view returns (uint256 nextGlobalEmissionDate){
        return lastGlobalEmissionDate.add(24 hours);
    }

    // function whitelistAddress(address _whitelistadd) public onlyOwner {}

    function poolLength() external view returns (uint256) {
        return poolInfo.length;
    }

    function add(IERC20 _lpToken, uint256 rewardBonusPer) public onlyOwner {
        poolInfo.push(
            PoolInfo({
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        dailyData[_pid][currentday].totalInvestment += _amount;
    ...
    function emitSetDailyData() public returns (bool emitted){
        if (hasEmissionHappenedEver) {
            if(now > lastGlobalEmissionDate + rewardDuration){
                setDailyData();
                return true;
            }else{
                return false;
    ...
    //only testing
    function doSetDailyData() public onlyOwner returns (bool emitted){
        if (hasEmissionHappenedEver) {
            if(now > lastGlobalEmissionDate + rewardDuration){
                setDailyData();
                return true;
    ...
    function setDailyData() internal returns (bool emitted){
        ...
        // if (hasEmissionHappenedEver) {
        //     require(lastGlobalEmissionDate + now > lastGlobalEmissionDate + rewardDuration ,"24 Hours to distribute rewards");
        // }
        ...
        for (uint256 j = 0; j < poolInfo.length; j++) {
            perDayToken = (poolInfo[j].rewardbonus.div(getPhase(phaseNumber + 1))).div(noOfPhaseDays);
            rewards = perDayToken;
            nextdayrewards = perDayToken;

            dailyData[j][currentday].totalrewards = rewards;

            // dailyData[j][currentday + 1].totalrewards = nextdayrewards;

            //take out extra tokens if there were no staking on previous day
            if (currentday != 0) {
                if (dailyData[j][currentday - 1].totalInvestment == 0) {
                    extraTokens += dailyData[j][currentday - 1].totalrewards;
                    dailyData[j][currentday - 1].totalrewards = 0;
                }
            }
        }
        lastGlobalEmissionDate = now;
        OngoingTotalRewardsPerDay = OngoingTotalRewards.div(getPhase(phaseNumber + 1)).div(noOfPhaseDays);
        emissionsPerPhase++;
    ...
    function getCurrentDay() public view returns (int256) {
        return currentday;
    }
        ...
        returns (uint256)
    {
        ...
        amount += (dailyData[_pid][currentday].totalrewards).mul(per).div(FLOAT_SCALAR);
        ...
        )
    {
        ...
        for (int256 j = user.lastrewardsettled; j < currentday; j++) {
            per = (user.amount).mul(FLOAT_SCALAR).div(dailyData[_pid][j].totalInvestment);
            _amount += (dailyData[_pid][j].totalrewards).mul(per).div(FLOAT_SCALAR);
        }
        return _amount;
    }
    ...
    function withdrawAll(uint256 _pid) public {
        ...
        uint256 amount;
        require(user.amount >= 0, "withdraw: not good");
        require(now >user.lastdepositetime + rewardDuration,"Min Stake time is 24 hours");
        ...
        user.unsettled = 0;
        user.lastrewardsettled = currentday;
        safedefitokentestTransfer(msg.sender, amount);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        // require(
        //     user.lastdepositetime + rewardDuration > now,
        //     "Min Stake time is 24 hours"
        // );
        uint256 settleamount = 0;
        if (user.amount > 0) {
            settleamount = settleIncome(_pid, msg.sender);
    ...
    function getDailyData(
        uint256 _pid,
        int256 day //  change method signature to accept pool name
    )
        public
        view
        returns (
    ...
    function safedefitokentestTransfer(address _to, uint256 _amount) internal {
        uint256 defitokentestBal = defitokentest.balanceOf(address(this));
        require(defitokentestBal >= _amount, "Low Balance ");
        defitokentest.transfer(_to, _amount);
    }
